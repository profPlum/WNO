...
PATH = 'data/ERA5_monthly_average.grib'
ntrain = 500
ntest = 20
...
sub = 6
h = int(((721 - 1)/sub)+1) # total grid size divided by the subsampling rate
s = int(((1441 - 1)/sub)+1)
...
x_train = data[:ntrain, ::sub, ::sub]
y_train = data[:ntrain, ::sub, ::sub]

x_test = data[-ntest:, ::sub, ::sub]
y_test = data[-ntest:, ::sub, ::sub]
...
model = WNO2d(width=width, level=level, layers=layers, size=[h,s], wavelet=wavelet,
              in_channel=in_channel, xgrid_range=xgrid_range, ygrid_range=ygrid_range, padding=1).to(device)
...
"""
torch.save(model, 'model/WNO_ERA5_t2m')
scipy.io.savemat('results/wno_results_ERA5_t2m.mat', mdict={'x_test':x_test.cpu().numpy(),
